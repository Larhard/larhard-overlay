...
if [[ ${WV} == "9999" ]] ; then
	...
	MY_P="${WN}-${WV/_/-}"
	SRC_URI="mirror://sourceforge/${WN}/Source/${MY_P}.tar.bz2"
...
LOL_PATCHES_N="lol-patches"
LOL_PATCHES_V="1.0"
LOL_PATCHES="${LOL_PATCHES_N}-${LOL_PATCHES_V}"

PULSE_PATCHES="winepulse-patches-1.7.20"
COMPHOLIOV="1.7.20"
COMPHOLIO_PATCHES="wine-compholio-daily-${COMPHOLIOV}"
WINE_GENTOO="wine-gentoo-2013.06.24"
DESCRIPTION="Free implementation of Windows(tm) on Unix patched to play League of Legends"
HOMEPAGE="http://www.winehq.org/"
SRC_URI="${SRC_URI}
	gecko? (
		abi_x86_32? ( mirror://sourceforge/${WN}/Wine%20Gecko/${GV}/wine_gecko-${GV}-x86.msi )
		abi_x86_64? ( mirror://sourceforge/${WN}/Wine%20Gecko/${GV}/wine_gecko-${GV}-x86_64.msi )
	)
	mono? ( mirror://sourceforge/${WN}/Wine%20Mono/${MV}/wine-mono-${MV}.msi )
...
LICENSE="LGPL-2.1"
SLOT="0"
IUSE="+abi_x86_32 +abi_x86_64 +alsa capi cups custom-cflags dos elibc_glibc +fontconfig +gecko gphoto2 gsm gstreamer +jpeg lcms ldap +mono mp3 ncurses netapi nls odbc openal opencl +opengl osmesa oss +perl pipelight +png +prelink pulseaudio +realtime +run-exes samba scanner selinux +ssl test +threads +truetype +udisks v4l +X xcomposite xinerama +xml +lol_patches"
...
src_unpack() {
	...
	if use lol_patches; then
		unpack "${LOL_PATCHES}.tar.gz"
	fi
...
src_prepare() {
	...
	local f
	local PATCHES=(
		"${FILESDIR}"/${WN}-1.5.26-winegcc.patch #260726
		"${FILESDIR}"/${WN}-1.4_rc2-multilib-portage.patch #395615
		"${FILESDIR}"/${WN}-1.7.12-osmesa-check.patch #429386
		"${FILESDIR}"/${WN}-1.6-memset-O3.patch #480508
	)
	use pulseaudio && PATCHES+=(
		"../${PULSE_PATCHES}"/*.patch #421365
	)
	use lol_patches && PATCHES+=(
		"../${LOL_PATCHES}"/*.patch #392940
	)
	if use pipelight; then
		PATCHES+=(
...
multilib_src_test() {
	...
	if [[ ${ABI} == x86 ]]; then
		if [[ $(id -u) == 0 ]]; then
			...
			ewarn "To run the test ${WN} suite, add userpriv to FEATURES in make.conf"
			return
		fi
